...
Whenever the Linux backend is installed, runnerize also registers a keep-awake holder that pings the WSL service every 30 seconds. This is not the same thing as keeping the host awake: WSL terminates an idle distro once no client session is attached, regardless of whether the host is asleep, so without the holder the Linux dispatcher is torn down shortly after every start.

Alongside each WSL logon trigger, runnerize registers an at-startup companion task that runs as the same user under an S4U principal, so an unattended reboot does not leave Linux runners dark until somebody signs in. These companions cannot use the SYSTEM account: a WSL2 distro is registered in its owning user's hive, and SYSTEM cannot resolve it. Whether S4U can load that hive before login is host-specific and depends on batch-logon policy and profile availability, so the companion is best-effort and logs its diagnostics to `%LOCALAPPDATA%\runnerize\runnerize-wsl-boot.log`. The logon triggers are unchanged and remain the primary path. Windows Sandbox remains unsupported while the desktop is locked or an RDP session is disconnected. AutoLogon still makes reboots fully unattended but stores reusable credentials and should be evaluated as a security trade-off.
